...
int enter_netns(const char *ns) {
  int cur_fd, net_fd;
  if ((cur_fd = open("/proc/self/ns/net", O_RDONLY)) == -1) {
    return -1;
  }

  if ((net_fd = open(ns, O_RDONLY)) == -1) {
    close(cur_fd);
    return -1;
  }
...
int main(int argc, char **argp) {
  ...
  int cur_fd;
  if ((cur_fd = enter_netns("/proc/374361/ns/net")) < 0) {
    return EXIT_FAILURE;
  }
  enter_netns("/proc/374361/ns/net");
